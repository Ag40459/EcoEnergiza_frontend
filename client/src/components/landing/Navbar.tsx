...
export default function Navbar({ onOpenLogin, theme, toggleTheme }: NavbarProps) {
  ...
  const solucoesData = [
    { 
      ...
      imagens: ["https://images.unsplash.com/photo-1509391366360-2e959784a276?auto=format&fit=crop&q=80&w=800"]
    },
    { 
      ...
      imagens: ["https://images.unsplash.com/photo-1558346490-a72e53ae2d4f?auto=format&fit=crop&q=80&w=800"]
    },
    { 
      ...
      imagens: ["https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?auto=format&fit=crop&q=80&w=800"]
    }
  ];
  ...
  const sobreData = [
    { 
      ...
      imagens: ["https://images.unsplash.com/photo-1497435334941-8c899ee9e8e9?auto=format&fit=crop&q=80&w=800"]
    },
    { 
      ...
      imagens: ["https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&q=80&w=800"]
    }
  ];
  ...
  return (
    <>
        ...
        <AnimatePresence>
          {menuAberto && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="lg:hidden bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 p-6 space-y-4"
            >
              <button onClick={() => { setModalSolucoes(true); setMenuAberto(false); }} className="block w-full text-left text-sm font-black text-[#004e3a] dark:text-gray-300 uppercase tracking-widest">Soluções</button>
              <button onClick={() => { setModalSobre(true); setMenuAberto(false); }} className="block w-full text-left text-sm font-black text-[#004e3a] dark:text-gray-300 uppercase tracking-widest">Sobre</button>
              <button onClick={() => { setModalContato(true); setMenuAberto(false); }} className="block w-full text-left text-sm font-black text-[#004e3a] dark:text-gray-300 uppercase tracking-widest">Contato</button>
              <button 
                onClick={() => { onOpenLogin(); setMenuAberto(false); }}
                className="w-full py-4 bg-[#004e3a] text-white rounded-2xl font-black text-xs uppercase tracking-widest"
              >
                Acessar Conta
              </button>
            </motion.div>
      ...
      <ContentModal 
        isOpen={modalSolucoes} 
        onClose={() => setModalSolucoes(false)} 
      ...
      <ContentModal 
        isOpen={modalSobre} 
        onClose={() => setModalSobre(false)} 
        title="Sobre a EcoEnergiza"
        data={sobreData}
      />
